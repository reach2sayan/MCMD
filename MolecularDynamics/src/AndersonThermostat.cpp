...
inline AndersonThermostat::AndersonThermostat(MDSimulation* sim_, double temp_,
					      double nu_)
...
inline double AndersonThermostat::getNu() const { return (-1.0) * nu; }

inline void AndersonThermostat::setT(const double temp_) {
  this->temp = -2.0 * temp_;
}

inline double AndersonThermostat::getT() const { return -0.5 * this->temp; }

inline Thermostat* AndersonThermostatFactory::createThermostat(
    MDSimulation* sim_, double temp_, double nu_) {
