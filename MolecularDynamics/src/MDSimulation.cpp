...
MDSimulation* MDSimulation::GetSimulationInstance(
    int dim, double dt, CalculatorType ctype, ThermostatType ttype,
    double r_inter, double r_verlet, int verletUpdate) {
  if (!instance)
    instance = new MDSimulation(dim, dt, ctype, ttype, r_inter, r_verlet,
				verletUpdate);
  return instance;
}

MDSimulation::MDSimulation(int dim_, double dt_, CalculatorType ctype,
			   ThermostatType ttype, double r_inter_,
			   double r_verlet_, int verletUpdate_)
    : particles(std::make_unique<MDParticleList>()),
      ...
      directional(
	  MatrixHistogram::Zero(HISTOGRAM_RESOLUTION, HISTOGRAM_RESOLUTION)),
      verletSteps(0),
  ...
  switch (ttype) {
    case ANDERSON:
      ...
      thermo = thermostatFactory->createThermostat(this, DEFAULT_TEMP_START,
						   DEFAULT_ANDERSON_NU);
  }
}

void MDSimulation::initSimulation(const bool periodic_, const Vector simBox_,
				  InitPositionFunc r0_, InitVelocityFunc v0_,
				  int histogramResolution_,
				  double histogramLength_) {
  histogramResolution =
      histogramResolution_ == 0 ? histogramResolution_ : HISTOGRAM_RESOLUTION;
  histogramLength = histogramLength_;
  simBox = simBox_;
  if (r0_) {
    int i = 0;
    for (auto& particle : *particles) particle->r = r0_(i++);
  } else {
  ...
  if (v0_) {
    int i = 0;
    for (auto& particle : *particles) particle->v = v0_(i++);
  } else {
    for (auto& particle : *particles) particle->v *= 0;
  }
  ...
  eKin = 0;
  for (const auto& particle : *particles) eKin += particle->v.squaredNorm();
...
void MDSimulation::velocityVerletStep(bool countRadial) {
  if (!pause) {
    for (auto& particle : *particles) {
      ...
      if (periodic) ApplyPeriodicBoundaryCondition(particle);
    }
    ...
    verletSteps++;
    for (auto& particle : *particles) particle->v += 0.5 * dt * particle->a;

    eKin = 0;
    for (auto& particle : *particles) eKin += particle->v.squaredNorm();
    eKin *= 0.5;
    t += dt;
  }
}

// has to be called after the velocityVerletStep function to extend the graphs
// by the last time step
void MDSimulation::updateGraphs() {
  if (pause) return;

  graphDataLast = (graphDataLast->next = new graphData_t);
  // The first list entry (graphDataFirst) contains the maximum values of the
  // graphs (for normalization). Assignment of new values and query for wether
  // they are greater than last maximum can be combined:

  graphDataLast->ePot = ePot;
  if (ePot > graphDataFirst->ePot) graphDataFirst->ePot = graphDataLast->ePot;

  graphDataLast->eKin = eKin;
  if (eKin > graphDataFirst->eKin) graphDataFirst->eKin = graphDataLast->eKin;

  graphDataLast->next = nullptr;
  if (ePot < ePotMin) ePotMin = ePot;
}
...
typename MDSimulation::MatrixHistogram
MDSimulation::getDirectionalDistribution() const {
...
double MDSimulation::refreshVerletLists(bool calc, bool countRadial) {
  ...
  Vector r_, f_;
  if (calc)
    for (auto& particle : *particles) particle->a = Vector::Zero(dim);
  ...
  for (auto entry = particles->begin(); entry != particles->end(); entry++) {
	...
	if (r_abs < hl) radial[static_cast<Eigen::Index>(r_abs / hl * hr)]++;
	if (dim == 2) {
	  if (abs(r_[0]) < hl && abs(r_[1]) < hl) {
	    directional(
		static_cast<Eigen::Index>((r_[0] / (2. * hl) + 0.5) * hr),
		static_cast<Eigen::Index>((r_[1] / (2 * hl) + 0.5) * hr))++;
	    directional(
		static_cast<Eigen::Index>((r_[0] / (-2. * hl) + 0.5) * hr),
		static_cast<Eigen::Index>((r_[1] / (-2. * hl) + 0.5) * hr))++;
	  }
      ...
      if (r_verlet > 0 && r_abs > r_verlet) continue;
...
double MDSimulation::velocityVerletForce() {
  ...
  for (auto& particle : *particles) particle->a *= 0;
  ...
  for (auto& particle : *particles) {
	...
	for (int i = 0; i < dim; i++) {
      ...
      if (r_inter_2 > 0 && (r_.squaredNorm() > r_inter_2)) continue;
...
void MDSimulation::log() const {
  for (auto logger : loggers) logger->log();
}
